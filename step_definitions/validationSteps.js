...
Then(/^(Element|Collection) "([^"]*)" should (not )?be (present|clickable|visible|invisible|selected|gone)$/, async (elementOrCollection, alias, negate, validation) => {
    ...
    if (elementOrCollection === "Collection") {
        const elements = await element.asElementFinders_();
        for (elem of elements) {
            await browser.wait(utils.ECHelper(elem, validation, negate), CUSTOM_TIMEOUT, `${elementOrCollection} ${alias} is${negate ? "" : " not"} ${validation}`);
        };
    } else {
...
Then(/^Count of "([^"]*)" elements should be equal "([^"]*)"$/, async (elements, count) => {
    const array = await parser.parser(elements).asElementFinders_();
    const arraysSize = array.length;
